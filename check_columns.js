...
async function check() {
  ...
  if (error) {
    console.log('Error fetching from usuarios:', error.message);
  } else if (data && data.length > 0) {
    console.log('Available columns in usuarios:', Object.keys(data[0]).join(', '));
  } else {
    console.log('No records found in usuarios to check columns.');
  }
}
check();
