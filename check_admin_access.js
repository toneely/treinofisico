...
async function check() {
  const { data, error } = await supabase.from('exercicios_padrao').select('id').limit(1);
  if (error) {
    console.log('Error access exercicios_padrao:', error.message);
  } else {
    console.log('exercicios_padrao is accessible');
  }
}
check();
