...
async function check() {
  const { error } = await supabase.from('usuarios').select('email_referencia').limit(1);
  if (error) {
    console.log('Error access email_referencia:', error.message);
  } else {
    console.log('email_referencia is accessible');
  }
}
check();
